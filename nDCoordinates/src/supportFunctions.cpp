...
void prog_opt_init(const int argc, const char *const argv[], params &par) {
    ...
    options.add_options()("help,h", "Display this help message")
            ...
            ("dimension", value<uint>(&(par.dim))->default_value(2), "(unsigned) Number of dimensions to sample the unit cube on.")
            ("numSamplePoints", value<uint>(&(par.numSamplePoints))->default_value(100), "(unsigned) Number of points to draw from [0,1]^d.")
