...
rule runCompilation:
    ...
    shell:
        "mkdir {output} && "
        "cd {output} && "
        "cmake -DCMAKE_CXX_FLAGS='{params.precisionOption}' ../../../{input}/nDCoordinates &&"
        "make"
...
rule collectDataPoints:
    ...
    params:
        ...
        dim=config['dim'],
        initSeed=getEntropyFiles
    threads: 1
    shell:
        "./{input} --numSamplePoints {params.numSamples} --dimension {params.dim} --outfile {output} --initSeed {params.initSeed}"
